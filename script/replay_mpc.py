...
for mpc in mpcs[START_NUM:]:
    ...
    ax5.plot(range(len(controls)), [s for t, s in controls])
    ax5.set_title("Steer commands")
    ax5.set_xlabel("Step")
    ax5.grid(True)

    ax2.plot(range(len(controls)), [t for t, s in controls])
    ...
    ax4.set_title(f"Simulation time differences between steps (mean Ts={round(mpc['mean_ts'], 4)})")
    ax4.grid(True)
