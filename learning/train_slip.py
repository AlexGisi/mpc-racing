...
DATA_TRAIN_FP = "data/big/slip_train.csv"
DATA_VAL_FP = "data/big/slip_validate.csv"
...
writer(f"MODEL={str(MODEL)}")

writer(f"Using device {DEVICE}.")
writer("Loading data...")

X_train, y_train = load_dataset(train_fp, FEATURES, TARGETS, DEVICE, DTYPE, drop_rows=1)
X_val, y_val = load_dataset(val_fp, FEATURES, TARGETS, DEVICE, DTYPE, drop_rows=1)
...
writer(f"y_min: {y_min}, y_max: {y_max}")
...
if MODEL != v.MLP and MODEL != v.MLP2:
    writer(initial_str)
    writer("Final report\n---")
    for n, p in model.named_parameters():
        writer(f"\t{n}: {p}")
